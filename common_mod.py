...
class C3_mamba(nn.Module):
    """
    Drop-in replacement for C3 with a Mamba branch.
    - keeps YOLOv5 C3 topology: two branches -> concat -> cv3
    - uses batched reshape to avoid Python loop over chunks
    - uses separate norms + gated fusion for stability
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=2,
                 d_state=16, d_conv=4, expand=2, chunks=4):
        super().__init__()
        assert chunks in (1, 2, 4, 8), "chunks should be a small power-of-two"
        c_ = int(c2 * e)
        assert c_ % chunks == 0, "hidden channels must be divisible by chunks"

        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1, 1)

        # keep local modeling (like original C3 bottlenecks)
        self.local = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(max(n, 1))))

        # Mamba over tokens
        self.norm1 = nn.LayerNorm(c_)
        self.norm2 = nn.LayerNorm(c_)
        self.chunks = chunks
        self.mamba = Mamba(
            d_model=c_ // chunks,
            d_state=d_state,
        ...
        # gated fusion (stabilize concat of different-normalized branches)
        self.gate = nn.Parameter(torch.tensor(0.0))

    def _mamba_tokens(self, x_bchw: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x_bchw.shape
        L = H * W

        # BCHW -> BLC
        x = x_bchw.view(B, C, L).transpose(1, 2).contiguous()  # (B, L, C)
        x = self.norm1(x)

        # batch reshape to avoid python for-loop:
        # (B, L, C) -> (B, L, chunks, C/chunks) -> (B*chunks, L, C/chunks)
        x = x.view(B, L, self.chunks, C // self.chunks)
        x = x.permute(0, 2, 1, 3).contiguous().view(B * self.chunks, L, C // self.chunks)

        x = self.mamba(x)  # (B*chunks, L, C/chunks)

        # back: (B*chunks, L, D) -> (B, L, C)
        x = x.view(B, self.chunks, L, C // self.chunks).permute(0, 2, 1, 3).contiguous().view(B, L, C)
        x = self.norm2(x)
    ...
    def forward(self, x):
        y1 = self.local(self.cv1(x))
        y2 = self.cv2(x)

        y_m = self._mamba_tokens(y1)
        alpha = torch.sigmoid(self.gate)  # scalar gate
        y1 = alpha * y_m + (1.0 - alpha) * y1  # gated residual mixing
...
class C3(nn.Module):
    ...
    def forward(self, x):
        ...
        a=self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
        #print(a.size())
        return a
...
class PatchPartition(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert H % self.patch_size == 0 and W % self.patch_size == 0, "Height and Width must be divisible by patch size"
...
class PatchCombine(nn.Module):
    ...
    def forward(self, x, h, w):
        b, n, p = x.shape
        c = p // (self.patch_size * self.patch_size)
...
class inject(nn.Module):
    def __init__(self, c1=448, c2=112, *args, **kwargs):
        super().__init__()
        # SS2D
        self.ss2d = SS2D(
            d_model=112, d_state=16, ssm_ratio=2.0, dt_rank="auto",
            act_layer=nn.SiLU, d_conv=3, conv_bias=True,
            dropout=0.0, bias=False,
            dt_min=0.001, dt_max=0.1, dt_init="random",
            dt_scale=1.0, dt_init_floor=1e-4,
            initialize="v0", forward_type="v2",
            channel_first=False,
            force_fp32=True,   # ✅ 数值更稳
        )

        # 用可学习下采样替换 AvgPool（信息保留更好）
        ...
        self.cv1 = Conv(448, 112, 1, 1)
        self.cv2 = Conv(112, 448, 1, 1)
        self.ln = nn.LayerNorm(112)

        # ✅ 真 depthwise-separable 替换 DWConv(gcd)
        self.mix = DWSeparable(448, 384, k=3, s=1, act=True)

        self.maxpool = nn.MaxPool2d(2, 2)

        # ✅ 注入门控（从 0 开始）
        self.gamma_s = nn.Parameter(torch.zeros(1))
        self.gamma_m = nn.Parameter(torch.zeros(1))

    def forward(self, x):
        x_s, x_m, x_l = x                 # (256@20), (128@40), (64@80)

        x_l40 = self.down_l(x_l)          # 64@40
        x_s40 = self.up_s(x_s)            # 256@40
        x_cat = torch.cat([x_l40, x_s40, x_m], dim=1)  # 448@40

        z = self.cv1(x_cat)               # 112@40
        ...
        x_cat = x_cat + z                 # residual

        y = self.mix(x_cat)               # 384@40
        y_s, y_m = torch.split(y, [256, 128], dim=1)

        y_s = self.maxpool(y_s)           # 256@20

        out_s = x_s + self.gamma_s * y_s
        out_m = x_m + self.gamma_m * y_m
        return out_s, out_m


class inject_up(nn.Module):
    def __init__(self, c1=384, c2=96, *args, **kwargs):
        super().__init__()
        self.ss2d = SS2D(
            d_model=96, d_state=16, ssm_ratio=2.0, dt_rank="auto",
            act_layer=nn.SiLU, d_conv=3, conv_bias=True,
            dropout=0.0, bias=False,   # ✅ 建议先别上 0.1
            dt_min=0.001, dt_max=0.1, dt_init="random",
            dt_scale=1.0, dt_init_floor=1e-4,
            initialize="v0", forward_type="v2",
            channel_first=False,
        ...
        self.upsample = nn.Upsample(scale_factor=2, mode="nearest")
        self.cv1 = Conv(384, 96, 1, 1)
        self.cv2 = Conv(96, 384, 1, 1)
        self.ln = nn.LayerNorm(96)

        self.mix = DWSeparable(384, 64, k=3, s=1, act=True)

        self.gamma_p3 = nn.Parameter(torch.zeros(1))  # ✅ 只注入到 P3

    def forward(self, x):
        (x_s, x_m), c_4 = x          # x_s:256@20, x_m:128@40, c_4:64@80

        x_s40 = self.upsample(x_s)   # 256@40
        x40 = torch.cat([x_s40, x_m], dim=1)   # 384@40
        x80 = self.upsample(x40)     # 384@80

        z = self.cv1(x80)            # 96@80
        z = z.permute(0, 2, 3, 1).contiguous()
        z = self.ln(z)
        z = self.ss2d(z)
        z = z.permute(0, 3, 1, 2).contiguous()
        ...
        x80 = x80 + z
        p3 = self.mix(x80)           # 64@80

        p3 = c_4 + self.gamma_p3 * p3
        return p3, x_m               # P3(64@80), P4(128@40)
